...
ABlock::ABlock()
{
	BlockValue = SpawnBlockWithChanceOfFour();
...
int32 ABlock::SpawnBlockWithChanceOfFour() const
{
	// Generate a random number between 0 and 99
	int32 RandomNumber = FMath::RandRange(0, 99);

	// If the random number is less than 10 (10% chance), spawn a block with value 4
	if (RandomNumber < 9)
	{
		return 4;
	}
